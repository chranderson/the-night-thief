...
* </code>
*/
angular.module('theNightThiefApp.decorators', ['firebase', 'currentAuth'])
  .config(['$provide', function($provide) {
    // adapt ng-cloak to wait for currentauth before it does its magic
    $provide.decorator('ngCloakDirective', ['$delegate', 'currentAuth',
      function($delegate, currentAuth) {
        var directive = $delegate[0];
        // make a copy of the old directive
        var _compile = directive.compile;
        directive.compile = function(element, attr) {
          currentAuth.getUser().then(function() {
